...
# Add 'overwight column
# if BMI > 25, overweight = 1
# if BMI <= 25, overweight = 0
# two possible values based on 1 condition
df['overweight'] = np.where(df['BMI'] > 25, 1, 0) 
print(df.head())

# Normalize data by making 0 always good and 1 always bad. If the value of 'cholesterol' or 'gluc' is 1, make the value 0. If the value is more than 1, make the value 1.

# Normailze cholesterol column (1 --> 0, 2 or 3--> 1)

# *** HOW TO REPLACE VALUES CONDITIONALLY! ***
df.loc[df['cholesterol']==1, 'cholesterol'] = 0
df.loc[df['cholesterol'] > 1, 'cholesterol'] = 1
# Normailze glucose column (1 --> 0, 2 or 3 --> 1)
...
def draw_cat_plot():
  ...
  # pd.melt gatehrs columns into rows
...
def draw_heat_map():
  ...
  # we check the [condition], identifying rows that are greater than the 2.5th percentile of the height data, then pull rows that meet the condition from df_heat; therefore, leaving out the two small values --> opposite for the next line
